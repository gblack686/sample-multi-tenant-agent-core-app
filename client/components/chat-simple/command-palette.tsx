...
export default function CommandPalette({ isOpen, onClose, onSelect }: CommandPaletteProps) {
    ...
    const info = filtered.filter((c) => c.category === 'info');
    const tools = filtered.filter((c) => c.category === 'tools');
    ...
    return (
        <div
            ...
            className="fixed inset-0 z-50 flex items-start justify-center pt-[20vh] bg-black/30 backdrop-blur-sm animate-in fade-in duration-150"
        >
            <div className="w-full max-w-lg bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-4 duration-200">
                {/* Header */}
