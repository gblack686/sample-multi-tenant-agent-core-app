...
/** All 28 test names emitted by the eval suite. */
const TEST_NAMES = [
  '1_session_creation',
  '2_session_resume',
  '3_trace_observation',
  ...
  '28_sdk_skill_subagent_orchestration',
];

const METRIC_NAMESPACE = 'EAGLE/Eval';

export interface EagleEvalStackProps extends cdk.StackProps {
  config: EagleConfig;
}

export class EagleEvalStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: EagleEvalStackProps) {
    super(scope, id, props);
    const { config } = props;

    // ── S3 bucket for eval artifacts ────────────────────────────
    const bucket = new s3.Bucket(this, 'EvalArtifactsBucket', {
      bucketName: config.evalBucketName,
    ...
    // ── CloudWatch Log Group (import existing) ──────────────────
    // The log group /eagle/test-runs is auto-created by the eval
    // suite on first run. Import it so CDK doesn't try to create
    // it (which would fail if it already exists).
    ...
    const alertTopic = new sns.Topic(this, 'EvalAlertTopic', {
      topicName: `eagle-eval-alerts-${config.env}`,
    ...
    const passRateAlarm = new cloudwatch.Alarm(this, 'EvalPassRateAlarm', {
      alarmName: `eagle-eval-pass-rate-${config.env}`,
    ...
    const dashboard = new cloudwatch.Dashboard(this, 'EvalDashboard', {
      dashboardName: `EAGLE-Eval-Dashboard-${config.env}`,
    });
    ...
    // Rows 2+: Per-test status widgets (6 per row, 28 total)
    ...
    new cdk.CfnOutput(this, 'EvalBucketName', {
      ...
      exportName: `eagle-eval-bucket-name-${config.env}`,
    });
    new cdk.CfnOutput(this, 'EvalBucketArn', {
      value: bucket.bucketArn,
      exportName: `eagle-eval-bucket-arn-${config.env}`,
    });
    new cdk.CfnOutput(this, 'EvalAlertTopicArn', {
      value: alertTopic.topicArn,
      exportName: `eagle-eval-alert-topic-arn-${config.env}`,
    });
    new cdk.CfnOutput(this, 'EvalDashboardName', {
      value: dashboard.dashboardName,
      exportName: `eagle-eval-dashboard-name-${config.env}`,
    });
    new cdk.CfnOutput(this, 'EvalPassRateAlarmArn', {
      value: passRateAlarm.alarmArn,
      exportName: `eagle-eval-alarm-arn-${config.env}`,
    });
  }
}
