...
8. LAUNCH AGENT
   ...
   IMPORTANT: Each agent will automatically run the Completion Sequence (Workflow 5)
   when done — validate, commit, push, create PR, update registry.

AFTER ALL COMPLETE:
- Report summary table to user (including PR URLs when available)
...
### 5. Agent Completion — Auto PR (MANDATORY)

**Every agent working in a worktree MUST complete this sequence before finishing.** This is not optional — the worktree lifecycle is incomplete without a PR.

**When the agent finishes its implementation work, it MUST:**

```
COMPLETION SEQUENCE (agent runs this automatically):

1. VALIDATE
   Run the project's validation suite before committing:
   - Python: ruff check app/ && python -m pytest tests/ -v
   - TypeScript: npx tsc --noEmit
   - Both: run whichever applies to files changed
   If validation fails, fix the issues first. Do NOT create a PR with failing checks.
...
4. CREATE PR
   Create a real (not draft) pull request:
   gh pr create \
   ...
   🤖 Generated with [Claude Code](https://claude.com/claude-code) via worktree-manager
   EOF
   )"

5. UPDATE REGISTRY
   After PR creation, update the registry with the PR number:
...
**If PR creation fails** (e.g., `gh` not authenticated, network error):
- Push the branch anyway so work is preserved
- Report the failure clearly with the error message
- The user can create the PR manually later
...
### Phase 1: Setup (orchestrator does this)
1. Detect project: `sample-multi-tenant-agent-core-app` (from git remote)
2. Detect package managers: `npm` (client/) + `pip` (server/)
3. Allocate 4 ports: `8100 8101 8102 8103`
4. Create worktrees, copy env files, install deps
5. Register both in `~/.claude/worktree-registry.json`
6. Launch agents in separate terminals
7. Report:
    ```
    ...
    | Branch | Ports | Task |
    |--------|-------|------|
    | feature/dark-mode | 8100, 8101 | Implement dark mode toggle |
    | fix/login-bug | 8102, 8103 | Fix login redirect bug |

    Agents launched. They will auto-create PRs when done.
    ```

### Phase 2: Agent works autonomously (each agent does this)
- Reads spec/plan from `.claude/specs/` if available
- Implements the feature/fix
- Runs validation (ruff, tsc, pytest as appropriate)

### Phase 3: Auto-completion (each agent does this automatically)
1. Commits with descriptive message
2. Pushes branch to origin
3. Creates PR via `gh pr create`
4. Updates registry with PR number
5. Reports: `✅ PR #42 created: https://github.com/.../pull/42`

### Phase 4: You review & clean up
- Review PRs on GitHub, merge
- Say "clean up merged worktrees" → skill removes worktrees, branches, releases ports

### End state
```
CREATE → WORK → VALIDATE → COMMIT → PUSH → PR → REVIEW → MERGE → CLEANUP
━━━━━━   ━━━━   ━━━━━━━━   ━━━━━━   ━━━━   ━━   ━━━━━━   ━━━━━   ━━━━━━━
skill    agent   agent      agent    agent  agent  you     you     skill
```
