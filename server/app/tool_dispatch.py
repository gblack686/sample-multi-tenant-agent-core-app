"""
EAGLE – Tool Dispatch
Handler functions for AWS-backed tools (S3, DynamoDB, CloudWatch, FAR search,
document generation, intake workflow) scoped per-tenant.

Exports: TOOL_DISPATCH, TOOLS_NEEDING_SESSION, _exec_create_document, and all
individual handler functions consumed by strands_agentic_service.py.
"""
import os
import json
import time
...
def _exec_create_document(params: dict, tenant_id: str, session_id: str = None) -> dict:
    ...
    try:
        from app.stores.reasoning_store import ReasoningLog as _RLog
...
def _exec_manage_skills(params: dict, tenant_id: str) -> dict:
    ...
    from app.stores.skill_store import (
...
def _exec_manage_prompts(params: dict, tenant_id: str) -> dict:
    ...
    from app.stores.prompt_store import (
...
def _exec_manage_templates(params: dict, tenant_id: str) -> dict:
    ...
    from app.stores.template_store import (
