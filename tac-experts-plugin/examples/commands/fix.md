...
PLAN_PATH: $2
REVIEW_PATH: $3
FIX_OUTPUT_DIRECTORY: `fix_reports/`
...
8. **Run Validation** - Execute validation commands:
   - `python -c "import py_compile; py_compile.compile('test_eagle_sdk_eval.py', doraise=True)"` - Syntax check
   - `python test_eagle_sdk_eval.py --model haiku --tests <relevant>` - Run affected tests
   - Any additional validation from the plan
